...
exports.sale = (req, res) => {
  ...
  const result = db.transaction(() => {
    ...
    const lines = items.map(i => {
      ...
      if (!p || !Number.isFinite(qty) || qty <= 0) throw bad('Producto o cantidad inválida');
      if (p.stock < qty) throw bad(`Stock insuficiente para ${p.nombre} (disponible: ${p.stock})`);
    ...
    for (const l of lines) {
      ...
      db.prepare('UPDATE productos SET stock=?,actualizado_en=CURRENT_TIMESTAMP WHERE id=?').run(next, l.p.id);
      db.prepare(`INSERT INTO movimientos_inventario(producto_id,tipo,cantidad,stock_anterior,stock_nuevo,usuario_id,referencia)
        VALUES(?,'venta',?,?,?,?,?)`).run(l.p.id, -l.qty, l.p.stock, next, req.session.user.id, numero);
    }
...
exports.purchase = (req, res) => {
  ...
  const result = db.transaction(() => {
    ...
    const lines = items.map(i => {
      ...
      if (!p || qty <= 0 || cost < 0) throw bad('Producto, cantidad o costo inválido');
